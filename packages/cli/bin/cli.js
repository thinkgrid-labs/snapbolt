...
if (!command || command === "--help" || command === "-h") {
  console.log(`
@thinkgrid/snapbolt-cli v${require("../package.json").version}
...
if (command === "scan") {
  ...
  if (!targetDir) {
    console.error(
      "❌ Error: Please specify a target directory (e.g., ./public)",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    const wasmPkgPath = path.dirname(
      require.resolve("@thinkgrid/snapbolt/package.json", {
    ...
    console.log(
      `💡 Note: Ensure your hook points to this location in production.`,
    );
  } catch (err) {
    console.error(`❌ Error: ${err.message}`);
    console.log(
      "\nTip: Make sure @thinkgrid/snapbolt is installed in your project.",
    );
    process.exit(1);
  }
} else {
