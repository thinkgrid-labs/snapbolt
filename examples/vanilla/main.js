...
fileQuality.addEventListener("input", () => {
  fileQualVal.value = fileQuality.value;
});
...
fileInput.addEventListener("change", async () => {
  ...
  try {
    ...
    showEl(fileOptImg);
    fileOptSize.innerHTML =
      fmt(blob.size) + " " + savings(file.size, blob.size);
...
urlQuality.addEventListener("input", () => {
  urlQualVal.value = urlQuality.value;
});
...
urlOptBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    showEl(urlOptImg);
    urlOptSize.innerHTML =
      fmt(blob.size) + " " + savings(origBytes.byteLength, blob.size);
  } catch (err) {
    hideEl(urlSkeleton);
    setError(
      urlError,
      `Failed: ${err.message}. Check CORS headers on the remote server.`,
    );
  } finally {
...
serverQuality.addEventListener("input", () => {
  serverQualVal.value = serverQuality.value;
});
